...
# -----------------------------
def processor_metronome(stop_event, freq=200, duration=0.2, sr=44100, bpm=60):
    # 음파 생성
    ...
    while not stop_event.is_set():
        ...
        sd.wait()
        time.sleep(bpm/60 - duration)  # BPM
...
sr = 44100
interval_s = 60 / bpm
t_met = np.linspace(0, duration, int(duration * sr))
click_sound = np.cos(2 * np.pi * 1000 * t_met)

def metronome_callback(outdata, frames, time_info, status):
    global is_first_call, click_pos, click_sound, interval_s, duration

    if status:
        print(status)
        
    outdata.fill(0.0)   # Zero-sound init
    # print(time_info)
    current_time = time_info.currentTime
    print(current_time, last_beat_time, interval_s)
    if current_time >= last_beat_time + interval_s:
        print("Tick")
        
        last_beat_time += interval_s
        click_pos = 0
    
    if click_pos < len(click_sound):
        copy_len = min(frames, len(click_sound) - click_pos)
        outdata[:copy_len] = click_sound[click_pos : click_pos + copy_len][:, np.newaxis]
        click_pos += copy_len

def start_metronome_stream(stop_event):
    # 메트로놈 재생 스트림을 백그라운드에서 실행
    metronome_stream = sd.OutputStream(
        samplerate=sr,
        channels=1,
        callback=metronome_callback
    )
    
    # 스트림을 열어 둡니다. 이 순간부터 콜백 함수가 호출되기 시작합니다.
    metronome_stream.start()
    
    # 메인 스레드나 녹음 스레드가 종료될 때 스트림을 닫아야 합니다.
...
def main():
    ...
    t = threading.Thread(target=processor_thread,
                         ...
                         daemon=True)
    metronome_stream = start_metronome_stream(stop_event)
    t.start()
    ...
    stop_event.set()
    t.join()
    metronome_stream.stop()
    metronome_stream.close()
